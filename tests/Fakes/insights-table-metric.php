...
/**
 * A stand-in for the analytics addon's base class for table-backed metrics.
 *
 * Different in kind from `insights-contracts.php` beside it, and the difference
 * is worth stating plainly. That file copies *signatures*: an interface has no
 * behaviour, so a faithful copy of its shape is a faithful copy of the whole
 * thing. This one copies an *implementation* — the windowing, the bucketing in
 * three SQL dialects, the split that keeps its null rows. Every metric in
 * `src/Integrations/Insights` is built on it, and since the sibling is a
 * `suggest` and is not installed here, this copy is what the entire suite
 * actually runs.
 *
 * Which means a copy that has drifted does not merely fail to compile. It
 * computes different numbers, quietly, and leaves the suite green — the exact
 * failure mode the contract guard was written to prevent, one layer down.
 *
 * So this file is a **verbatim** copy of
 * `statamic-insights/src/Support/TableMetric.php` from the `use` statements
 * down, and `InsightsContractsMatchTest` compares the two as text rather than
 * by reflection. Reflection would compare names and types and would not notice
 * a changed `where`. If you touch anything between the markers, you have
 * changed what this addon is tested against; copy the upstream file across
 * again instead.
 *
 * Guarded like its neighbour, so a real installation wins and this becomes
 * inert. Loaded by hand from the tests that need it, never through an
 * autoload path — see `insights-contracts.php` for why.
 *
 * Pint leaves the copy alone as it stands. Should a future release of it want
 * to reformat the block, exclude this file rather than letting it — the
 * comparison is what makes the copy worth having, and a reformatted copy fails
 * it while saying "copy it across again", which would send whoever reads that
...
if (! class_exists('Goldnead\StatamicInsights\Support\TableMetric')) {
    // >>> BEGIN VERBATIM COPY OF statamic-insights/src/Support/TableMetric.php
/**
